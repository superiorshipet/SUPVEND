...
export default function Auctions() {
  ...
  useEffect(() => {
    auctionAPI.getActive().then((r) => setAuctions(r.data?.data?.auctions || r.data?.auctions || [])).catch(() => {}).finally(() => setLoading(false));
  }, []);
  ...
  const columns = [
    ...
    { key: 'currentBid', label: 'Current', render: (v) => formatCurrency(v || 0) },
    { key: 'bids', label: 'Bids', render: (v) => v?.length || 0 },
